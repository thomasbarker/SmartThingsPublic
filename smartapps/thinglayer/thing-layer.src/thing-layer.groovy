...
private logField(evt, Closure c) {
    ...
    def cleanName = evt.displayName.replaceAll("\\s","-")
    
    //httpPostJson(uri: "https://136powers.firebaseio.com/events/${evt.deviceId}/${evt.name}.json",   body:[device: evt.deviceId, name: evt.name, value: evt.value, date: evt.isoDate, unit: evt.unit]) {
    //httpPostJson(uri: "https://136powers.firebaseio.com/events/${cleanName}/${evt.name}.json",   body:[name: evt.name, value: evt.value, date: evt.isoDate, unit: evt.unit]) {
      httpPostJson(uri: "https://136powers.firebaseio.com/events/${evt.name}/${cleanName}.json",   body:[name: evt.name, value: evt.value, date: evt.isoDate, unit: evt.unit]) {
      
      //log.debug "posting to https://136powers.firebaseio.com/events/${cleanName}.json" + cleanName + ":::" + evt.name+ ":" + evt.descriptionText + ":" + evt.displayName + ":" + "Event data successfully posted"
    }
}